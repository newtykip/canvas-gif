...
interface Options {
	...
	/**
	 * How many channels the GIF has - only input this yourself if you know what you are doing
	 * @default the amount of channels in the source gif (or 4)
	 */
	channels?: number;
}
...
type Repeat<T extends number = number> = Positive<T> | 'forever';
...
class Gif {
	...
	static async fromBuffer(buffer: Buffer, options?: Options) {
		...
		const gif = new Gif(width, height, frames.length, {
			...
			channels: options?.channels ?? channels ?? defaultOptions.channels,
		});

		for (let i = 0; i < frames.length; i++) {
			gif.frames[i].sharp = sharp(frames[i], {
				raw: {
					width,
					height,
	...
	constructor(
		...
		this.channels = this.options?.channels ?? defaultOptions.channels;
